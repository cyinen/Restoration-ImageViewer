...
sub_num, sub_colors = 4, [(0, 0, 255), (0, 255, 0), (255, 0, 0),(0, 100, 0)]
...
while True:
    ...
    win_image = cv2.imread('%s/%s' % (result_paths[main_index], image_name))
    image_h, image_w, _ = win_image.shape
    wimage_h, wimage_w = win_h, int(image_w * (win_h / image_h))
    wimage = cv2.resize(win_image, (wimage_w, wimage_h), interpolation=cv2.INTER_CUBIC)
